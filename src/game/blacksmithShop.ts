...
const DEFAULT_BLACKSMITH_INVENTORY_GOLD = 5000
...
export const getBlacksmithShopSellPriceById = (
  ...
  const buyPrice = getBlacksmithShopItemPriceById(itemId)
...
export const sellBlacksmithShopItem = ({
  ...
  const emptyMerchantSlotIndex = findFirstEmptyPlayerInventorySlotIndex(
    merchantInventory
  )

  if (emptyMerchantSlotIndex === undefined) {
    return createBlacksmithShopTransactionFailure({
      playerInventory,
      merchantInventory,
      message: '상점 재고가 가득 찼습니다.'
    })
  }
  ...
  const nextMerchantInventory = withInventoryGold(
    setPlayerInventorySlot({
      inventory: merchantInventory,
      slotIndex: emptyMerchantSlotIndex,
      item: clonePlayerInventoryItem(playerItem)
    }),
    merchantInventory.gold - itemPrice
  )
