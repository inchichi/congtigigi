...
// 복원은 디스크의 src/assets 파일을 원본 바이트로 되써, Vite가 게임(iframe)을 자동 리로드한다.
...
export const createStyleRevertControl = (): StyleRevertControl => {
  ...
  const assetName = (path: string): string => path.replace(/^src\/assets\//, '')
  ...
  const renderList = (): void => {
    ...
    // Vite가 src/assets를 그대로 서빙한다. 현재(스타일 적용된) 파일을 캐시버스트로 받는다.
    picker.setItems(
      assets.map((asset) => ({
        id: asset.path,
