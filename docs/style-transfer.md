...
- `src/editor/createStyleTransferModal.ts` — 헤더 🎨 버튼 + 모달 UI.
  콘텐츠는 세 가지 — ① 파일 업로드 ② 게임 에셋(`src/games/my-sample-rpg/assets`의 PNG 목록에서 선택)
...
- `style-service/asset_store.py` — 쓰기 게이트: `src/games/my-sample-rpg/assets` 밖 경로 차단, 덮어쓰기 전
...
- 저장 위치가 `src/games/my-sample-rpg/assets`가 아닌 이유: Vite가 src/ 변경을 감지해 페이지를 리로드하므로,
...
## 게임에 적용 (src/games/my-sample-rpg/assets 덮어쓰기)

`🕹 게임에 적용`은 결과를 `src/games/my-sample-rpg/assets`의 원본 PNG에 덮어쓴다(게임 에셋 모드: 선택한 파일,
...
- 서비스는 `src/games/my-sample-rpg/assets` 밖 경로·PNG 외 형식·존재하지 않는 파일을 거부하고,
