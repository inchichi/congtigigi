...
- `src/editor/createEditorApp.ts`: the editor IDE shell — a three-pane layout (project entity tree, generation panel, live game preview iframe) styled by `src/editor.css` (Tailwind only). Drives open-folder, LLM analysis, generation, validation, apply, and export.
...
- `src/editor/pendingEvents.ts`: the localStorage handoff — the editor saves generated events and the game applies them on scene boot and on a `storage` event (live, no reload).
