...
- `src/editor/createEditorApp.ts`: the editor IDE shell — a three-pane layout (project entity tree, generation panel, live game preview iframe) styled by `src/editor.css` (Tailwind only). Drives open-folder, LLM analysis, generation, validation, apply, export, an in-session generation history, and reset-to-bundled-game.
