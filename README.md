...
- 동적(생성) 퀘스트를 Lua 퀘스트 런타임에도 주입·동기화 — 퀘스트 추적 팝업·퀘스트 창·NPC 상호작용이 생성 퀘스트를 인식 (등록/해제는 Lua 파사드 경유로 일원화)
- Lua 트래커가 `objective` 필드를 누락해 TS와 불일치하던 버그 수정
- 진행 중인 "아이템 획득" 목표가 있으면 몬스터가 해당 아이템을 우선 드랍 (드랍 확률은 기존 유지)
- 에셋 스타일 전이가 항상 최초 원본에서 시작하도록 변경(이전 결과 위 덧칠 누적 차단) + 검은 손상 결과 차단 게이트를 에셋 경로까지 확장
- `POST /api/style/stylize-sheet` 추가 — NPC·몬스터 시트를 프레임 단위로 전이 후 재조립(프레임 손상 시 전체 중단)
- 퀘스트 등 게임에 노출되는 생성 텍스트(제목·대사·안내·목표 라벨·아이템 이름)를 전면 한국어로 전환
- 생성 퀘스트가 새로고침에도 유지되고, 에디터 헤더의 `↩ 퀘스트 되돌리기` 버튼으로만 제거되도록 변경
- 스타일 대상 수동 편집: 추천 대상 개별 제외, 카탈로그 전체(64개)에서 직접 추가, 전체 선택/해제, 추가분 공통 프롬프트·강도 지정
- 변환 결과 요약 페이지(`editor.html?workspace=summary`): 적용 이력 JSON 일괄/개별 다운로드 + 스타일 전이 전/후 타일 비교·저장
- 희소 컷아웃(나무·가로등 등) 검은 오염 수정: FLUX 입력의 투명 배경을 중립색으로 평탄화 + 손상 결과의 타일셋 반영을 차단하는 품질 게이트
- 일부 스타일 대상이 실패해도 나머지 대상과 퀘스트 저장을 계속 진행하고, 실패 목록을 화면·작업 로그에 기록
- 에디터 부팅 자동 동기화에 60초 쿨다운을 넣어 장시간 배치 중 새로고침 반복 루프 제거
- FLUX 프롬프트에서 대상 지시를 맨 앞으로 재배치(CLIP 77토큰 잘림 대응)
- 퀘스트 단계가 후보 2개를 병렬 생성하고, 읽기 쉬운 카드(제목·대사·목표·보상)로 비교 후 선택하는 방식으로 변경
- 신규 보상 아이템 생성 옵션: Qwen이 아이템 데이터(id·라벨·기준 아이콘·프롬프트)를 설계하고 FLUX가 기존 무기/방어구 아이콘을 변형해 `assets/weapons/generated/`에 저장, 퀘스트 보상으로 지급
- dev 엔드포인트 `POST /__save-generated-asset` 추가(생성 아이콘 로컬 저장, 게임 에셋 경로로 제한)
- 기말 원본 기준으로 오염된 에셋 14종(타일셋·무기·이펙트)과 대장장이 NPC 이름 복원
